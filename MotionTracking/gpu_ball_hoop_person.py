...
player_recheck_interval = 1.0  # in seconds
...
def detect_and_track_basketball_and_players(image, force_redetect=False):
    ...
    global last_player_check_time  # new
    ...
    # YOLO detection for ball (if not tracking)
        ...
        for result in ball_results:
            for box in result.boxes:
                ...
                if class_id == 0 and confidence > 0.70:
                    ...
                    cv2.putText(image, f"Basketball: {confidence:.2f}", (x1, y1 - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                    cv2.circle(image, ball_center, 5, (0, 255, 255), -1)
                    break
            if tracking_ball:
                break

    # ⏱️ Periodic YOLO detection for players
    ...
    return tracking_ball, ball_center, player_boxes
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                ball_detected, ball_center,player_box = detect_and_track_basketball_and_players(frame, force_redetect)
